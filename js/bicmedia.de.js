...
function doBICMSearch() {
	...
	if(
		...
		j("#message").html("Geben Sie f&uuml;r die Suche mindestens eine ISBN Nummer oder einen Titel, Autor oder Verlag an.");
		return false;
	} 
...
function doAJAXSearchRequest(value) {
	j("#message").html("Suche wird ausgef&uuml;hrt");
...
function parseResult(data, textStatus) {
	...
	if(fehler.length != 0) {
		j("#message").html("<b style='color:red'>Ein Fehler ist aufgetreten <br />"+fehler+"</b>");
		return false;
	} 
	else if (j(data).find("book").length == 0 && j(data).find("audio").length == 0) {
		j("#message").html("Keine Ergebnisse gefunden");
		return false;
	} else {
		j("#message").html("Suche erfolgreich");
...
function parseResultISBN(data, textStatus) {
	...
	if(fehler.length != 0) {
		...
		var auswahl = j("<a></a>").html("&Uuml;bernehmen").click(function () { insertResultTag(isbn); }).css("cursor","pointer");
...
function fillTable(target, data) {
	...
	var auswahl = j("<a></a>").html("Auswahl").click(function () { insertResultTag(isbn); }).css("cursor","pointer");
